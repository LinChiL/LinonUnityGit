...
public class FootIKController : MonoBehaviour
{
    ...
    public LayerMask groundLayer;      // 手动指定地面层
    ...
    {
        // 1. 基于稳定目标发射射线
        Vector3 rayStart = stableTarget.position + Vector3.up * 0.3f; // 射线起点在目标上方
        ...
        if (Physics.Raycast(ray, out RaycastHit hit, raycastDistance, groundLayer, QueryTriggerInteraction.Ignore))
        {
            ...
            newTargetPos.x = stableTarget.position.x; // X轴跟随稳定目标
            newTargetPos.z = stableTarget.position.z; // Z轴跟随稳定目标

            // 3. 计算目标旋转（地面法线 + 稳定目标的Y轴旋转）
            ...
            Quaternion newTargetRot = yawRot * normalRot; // 组合旋转
